...
public class BaseballElimination { 
  ...
  private final Bag<String>[] eliminateTeams;
  ...
  public BaseballElimination(String filename) {
    ...
    eliminateTeams = (Bag<String>[])new Bag[V];
  ...
  private boolean eliminate(String team) {
    ...
    int sum = 0, gameIndex = 1, verticesIndex = vertices - V;
    ...
    for(int i = 0; i < V; i++) {
      ...
      if(tarTeam.wins + tarTeam.remain - curTeam.wins < 0) {
        ...
        eliminateTeams[tarIndex] = new Bag<String>();
        eliminateTeams[tarIndex].add(names[i]);
        return true;
      }
      ...
      for(int j = i + 1; j < V; j++) {
        ...
        G.addEdge(new FlowEdge(gameIndex, i + vertices - V, Double.POSITIVE_INFINITY));
        G.addEdge(new FlowEdge(gameIndex, j + vertices - V, Double.POSITIVE_INFINITY));
        gameIndex++;
      }
      
      G.addEdge(new FlowEdge(verticesIndex++, vertices - 1, tarTeam.wins + tarTeam.remain - curTeam.wins));
    }

//    System.out.println("at the end of game loop, gameIndex = vertices - V");
//    System.out.println("in this case vertices=" + vertices + ", V=" + V + ", and gameIndex is: " + gameIndex);
    
    FordFulkerson ff = new FordFulkerson(G, 0, vertices - 1);
    
    if(ff.value() == sum) {
      isEliminated[tarIndex] = 2;
      eliminateTeams[tarIndex] = null;
      return false;
      ...
      for(int i = gameIndex; i < vertices - 1; i++) {
        if(ff.inCut(i) && i - gameIndex != tarIndex) 
          eliminateTeams[tarIndex].add(names[i - gameIndex]);
      }
