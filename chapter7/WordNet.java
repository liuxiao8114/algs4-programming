...
public class WordNet {
   ...
   public WordNet(String synsets, String hypernyms) {
      ...
      this.nouns = (Bag<String>[])new Bag[length];
      this.ids = new ST<String, Bag<Integer>>();

      for(int i = 0; i < length; i++) {
        String[] temp = lines[i].split(",")[1].split(" ");
        for(int j = 0; j < temp.length; j++) {
        	this.nouns[i] = new Bag<String>();
        	this.nouns[i].add(temp[j]);
   ...
   public String sap(String nounA, String nounB) {
	   ...
	   return nouns[sap.ancestor(ids.get(nounA), ids.get(nounB))].iterator().next();
   }
